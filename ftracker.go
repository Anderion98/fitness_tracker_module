...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	valueMsec := meanSpeed(action, duration) * kmhInMsec
	value := height / 100
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(valueMsec, 2)/value)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
